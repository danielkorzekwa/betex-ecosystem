...
public class EsperMarketTradedVolumeAvgPricesBettingSimulationTestApp {
	...
	//private long marketId = 101095109l;
	// private long marketId = 101095121l;
	...
	public void testProcessMarketTradedVolume() {
		...
		for (int i = 0; i < marketTradedVolumeList.getRows().size(); i++) {
			ValueAndDocumentRow<BaseDocument, MarketTradedVolume> marketTradedVolumeRow = marketTradedVolumeList
					.getRows().get(i);
			ValueAndDocumentRow<BaseDocument, MarketPrices> marketPricesRow = marketPricesList.getRows().get(i);

			MarketDataEvent marketDataEvent = new MarketDataEvent(marketTradedVolumeRow.getDocument(), marketPricesRow
					.getDocument());
		...
		while (marketTradedVolumeList.getRows().size() > 0 && marketPricesList.getRows().size() > 0) {
			...
			for (int i = 0; i < marketTradedVolumeList.getRows().size(); i++) {
				...
				MarketDataEvent marketDataEvent = new MarketDataEvent(marketTradedVolumeRow.getDocument(),
						marketPricesRow.getDocument());
	...
	private class EventLister implements UpdateListener {
		...
		public void update(EventBean[] newEvents, EventBean[] oldEvents) {
			...
			for (HeatMapColumn column : modelDelta.getColumns()) {
				...
				runnerProbs.put(selectionId, runnerProb);
				marketProbs.put(marketId, new MarketProb(marketId, runnerProbs));

				Bet bet = null;
				if (column.getTotal() > 0 && priceFactor > 1.03) {
					bet = new Bet(marketId, selectionId, 2, bestToBack);
				}
				if (column.getTotal() > 0 && priceFactor < 0.95) {
					bet = new Bet(marketId, selectionId, -2, bestToLay);
				}
				// System.out.println(newEvents.length + ":"
				// + new Date(lastEvent.getMarketTradedVolume().getTimestamp()) + ":"
				// + ":" + round(column.getTotal(), 2) + ":"
				// + round(avgPrice, 2) + ":" + runnerPrices.getLastPriceMatched() + ":"
				// + round(bestToBack, 2) + ":" + round(bestToLay, 2) + ":" + round(priceFactor, 2));

				if (lastEvent.getMarketPrices().getInPlayDelay() == 0 && (1 / runnerProb) < 5 && bet != null) {
					bets.add(bet);
					System.out.println(bet);
				}
			}

			if (lastEvent.getMarketPrices().getInPlayDelay() > 0) {
				System.out.println("Market in play.");
			}
			System.out.println(new Date(lastEvent.getMarketTradedVolume().getTimestamp()) + ":" + bets.size() + ":"
